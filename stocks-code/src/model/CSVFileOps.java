...
/**
 * The CSVFileOps class which is used to Read/Write CSV files and perform data check operations (like validating the
 * Stock,Quantity which our application ingests.
 */
public class CSVFileOps {
    ...
    /**
     * The default constructor for our class which just initializes the current OS's path separator (\\ for windows and
     * / for linux).
     */
    public CSVFileOps() {
    ...
    /**
     * Reads a CSV file given it's filename and dir. By default, it's only designed to look inside ./app_data/ as a
     * security feature. The 'dir' is the name of the folder sitting inside './app_data'.
     *
    ...
    /**
     * Writes given CSV data (as String) into a file with given 'filename' and 'dir' parameters. By default the 'dir'
     * provided will be created inside "./app_data/", there's no option given to change this for security measures.
     *
    ...
    /**
     * Our internal data validator method that checks for CSV data that our application can ingest (like only integer
     * stock quantities, String scrip names that match a regex to name a few). It discards missing data rows from the
     * CSV file.
     *
     * @param data a list of String of data containing comma separated column values (Stock,Quantity generally)
     */
    ...
    /**
     * Our internal method to check if the given Stock Ticker is a valid ticker or not. It follows the rule:
     * Atleast 1 alphabet/number followed by an optional dot followed by atleast one alphabet. We used this to include
     * stocks from other stock exchanges.
     *
    ...
    /**
     * Internal method to construct a complete path to file, given a path and directory (sitting inside './app_data').
     *
