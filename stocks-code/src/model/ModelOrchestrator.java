...
/**
 * Our model orchestrator class, as the name suggests, it is the link for the controller to call the internal model
 * classes and provide abstraction to the functionalities of our application. It implements the Orchestrator interface.
 */
public class ModelOrchestrator implements Orchestrator {
    ...
    /**
     * Takes in (Stocks,Quantity) CSV data, generates a random portfolio ID and then saves it (CSV data) in a file.
     *
    ...
    /**
     * Fetches Portfolio Value as CSV Data in string format (stock,quantity,value) given a date and portfolio data
     * (in CSV). Since weekend data is unavailble for stocks, returns null if given date is a weekend.
     *
...
//    Date dateNew = DateFormatter.getDate(date);
    ...
    /**
     * Shows the existing portfolios in './app_data/PortfolioData' where our application is programmed to store all
     * Portfolios. Also creates the mentioned directory if not present.
     *
